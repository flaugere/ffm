...
export default class JP7 {
    ...
    static calculate(tricep: number, suprailiac: number, thigh: number, abdominal: number, chest: number, axillary: number, subscapular: number, age: number): number {
        let sumSkinFolds = tricep + suprailiac + thigh + abdominal + chest + axillary + subscapular;
        let bodyDensity = 1.097 - (0.00046971 * sumSkinFolds) + (0.00000056 * sumSkinFolds * sumSkinFolds ) - (0.00012828 * age);
